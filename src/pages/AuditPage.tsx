...
export function AuditPage() {
  ...
  return (
    <section className="rounded-[2.25rem] border border-[var(--border-soft)] bg-[var(--surface)] p-6 shadow-[var(--shadow-soft)]">
      ...
      <div className="mt-6 space-y-4">
        ...
        {auditQuery.data?.map((event) => (
          <article key={`${event.routingKey}-${event.entityId}-${event.timestamp}`} className="rounded-[1.75rem] border border-[var(--border-soft)] bg-white p-5 shadow-[0_18px_40px_rgba(31,58,55,0.05)]">
            ...
            <pre className="mt-4 overflow-x-auto rounded-2xl border border-emerald-100 bg-emerald-50/50 p-4 text-sm text-slate-700">
